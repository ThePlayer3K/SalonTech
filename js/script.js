...
function setupServices() {
    ...
    renderServices(appStore.services);
...
function renderServices(items) {
    ...
    grid.innerHTML = items.map((service) => {
        ...
        return `<article class="service-card"><div class="service-meta"><span class="badge">${escapeHtml(formatCategory(service.categoria))}</span><span class="price">${formatCurrency(Number(service.valor))}</span></div><h3>${escapeHtml(service.nome)}</h3><small>${Number(service.duracaoMin)} min com ${escapeHtml(professional?.nome||"Equipe")}</small><button class="btn btn-primary" type="button" data-service-interest="${service.id}">Usar na agenda</button></article>`;
    }).join("");
    grid.querySelectorAll("[data-service-interest]").forEach((btn) => btn.addEventListener("click", () => {
        const selected = appStore.services.find((s) => s.id === btn.dataset.serviceInterest);
        if (!selected) return;
        ...
        console.log(`${selected.nome} enviado para a agenda.`);
    }));
}
...
function applyServiceFilters() {
    ...
    const price = ($("#priceFilter")?.value)||"todos";
    const filtered = appStore.services.filter((s) => {
        const pro = appStore.team.find((t) => t.id === s.profissionalId);
        const matchSearch = [s.nome,s.categoria,pro?.nome].join(" ").toLowerCase().includes(search);
        const matchCategory = category==="todos" || s.categoria===category;
        const matchPrice = price==="todos" || (price==="ate40"&&Number(s.valor)<=40) || (price==="41a80"&&Number(s.valor)>=41&&Number(s.valor)<=80) || (price==="acima80"&&Number(s.valor)>80);
...
function renderServiceOptions() {
    ...
    const cs = $("#contactService"); const rs = $("#reviewService");
    if (cs) cs.innerHTML = opts; if (rs) rs.innerHTML = opts;
}
...
function renderField(field) {
    ...
    if (type === "select") {
        ...
        let opts = field.options ? field.options.map((o) => { const l=typeof o==="object"?o.label:o; const v=typeof o==="object"?o.value:o; return `<option value="${escapeHtml(v)}">${escapeHtml(l)}</option>`; }).join("") : (appStore[field.source]||[]).map((r) => `<option value="${escapeHtml(r.id)}">${escapeHtml(r[field.optionLabel]||r.nome||r.id)}</option>`).join("");
...
function renderAdminRecords(records) {
    ...
    list.innerHTML = records.map((r) => {
        ...
        const details = currentAdminModule==="appointments" ? `<span>Data: ${formatDate(r.data)} ${r.hora||""}</span><span>Valor: ${formatCurrency(Number(r.valor||0))}</span><span>Situação: ${r.situacao}</span>` : currentAdminModule==="expenses" ? `<span>Data: ${formatDate(r.data)}</span><span>Valor: ${formatCurrency(Number(r.valor||0))}</span>` : "";
...
async function handleAdminSubmit(event) {
    event.preventDefault(); if (currentAdminModule === "finances") return;
...
function toDatabasePayload(storeKey, data) {
    ...
    const map = {
        clients: { nome:data.nome, cpf:n(data.cpf), telefone:n(formatPhone(data.telefone)), email:n(data.email) },
        appointments: { cliente_id:Number(data.clienteId), forma_pag_id:nid(data.formaPagamentoId), situacao:data.situacao, data_hora:`${data.data}T${data.hora||"00:00"}:00`, observacoes:n(data.observacoes), servico_id:Number(data.servicoId), valor_total:Number(data.valor||0) },
        ...
        team: { nome:data.nome, cpf:n(data.cpf), telefone:n(formatPhone(data.telefone)), cargo:n(data.cargo), tipo_pagamento:n(data.tipoPagamento), salario:Number(data.salario||0) },
        products: { nome:data.nome, qtd_estoque:Number(data.quantidade||0), categoria:n(data.categoria) },
...
function editAdminRecord(id) {
    const mod = adminModules[currentAdminModule]; const record = (appStore[mod.storeKey]||[]).find((r) => r.id === id); if (!record) return;
    editingRecordId = id; renderAdminModule();
    mod.fields.forEach((field) => { const input = $(`#admin-${field.name}`); if (input) input.value = record[field.name]??""; });
    $("#adminSubmitBtn").textContent = "Salvar alterações"; $("#adminCancelEditBtn").hidden = false;
...
async function deleteAdminRecord(id) {
    ...
    try { const { error } = await supabase.from(table).delete().eq("id", Number(id)); if (error) throw error; await loadStoreFromSupabase(); renderServices(appStore.services); editingRecordId = null; renderAdminModule(); showToast("Cadastro excluído."); }
